...
public class CreateCrmItemTests : BaseTestFixture
{
    [Test]
    ...
    public async Task ShouldCreateCrmItemWithNoFields()
    {
        ...
        var crmTypeCommand = CrmTypeTests.GetCreateTypeCommand();
        await SendAsync(crmTypeCommand);

        var entity = await SendAsync(new CreateCrmItemCommand
        {
            Type = crmTypeCommand.Id
        });
    ...
    public async Task ShouldCreateCrmItemWithOneField()
    {
        ...
        entity.Result!.Type.Should().Be(crmTypeCommand.Id);
